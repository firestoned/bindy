...
| Control | Implementation | Evidence Location |
|---------|----------------|-------------------|
| **Least Privilege RBAC** | Operator has minimal RBAC (create/delete secrets for RNDC lifecycle, delete managed resources for cleanup) | [`deploy/rbac/role.yaml`](../../../deploy/rbac/role.yaml) |
...
1. **RBAC Policy**: [`deploy/rbac/role.yaml`](../../../deploy/rbac/role.yaml) - Shows minimal required permissions with detailed rationale
...
kubectl logs -n bindy-system -l app=audit-log-verifier --since 24h
