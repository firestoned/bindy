  ...
  global:
    dnssec:
      signing:
        ...
        exportToSecret: true  # Controller exports to Secret for backup
```

**How it Works:**
1. BIND9 generates keys in `/var/cache/bind/keys/` (emptyDir)
2. Controller watches for new keys via bindcar API
3. Controller exports keys to Secret `dnssec-keys-<zone>-generated`
4. On pod restart: Controller restores keys from Secret to emptyDir
...
**Benefits:**
- **No Controller Dependency**: Keys persist independently of controller
...
- **Auto-Generated**: BIND9 handles rotation based on `dnssec-policy`, controller updates Secret
...
2. Controller checks for Secret with keys (`dnssec-keys-<zone>`)
3. **If Secret exists**: Restore keys from Secret to emptyDir
4. **If Secret missing**: BIND9 regenerates keys, controller exports new keys to Secret
5. Controller updates DNSZone status with new DS records
   ...
   if is_dnssec_signing_enabled(&global_config, instance_config) {
       ...
       match &signing_config.keys_from {
           ...
           None if signing_config.auto_generate.unwrap_or(true) => {
               ...
               // Controller will export keys to Secret if exportToSecret is true
     ...
     global:
       dnssec:
         ...
         signing:
           enabled: true
           autoGenerate: true         # BIND9 generates keys
           exportToSecret: true        # Controller exports to Secret for restore
           policy: "default"
           algorithm: "ECDSAP256SHA256"
...
2. **Controller Compatibility**: DNSSEC logic only applied if feature enabled
...
- **✅ Automatic Recovery**: If Secret is deleted, BIND9 auto-generates new keys, controller exports to new Secret
...
- Monitor key rollover events in controller logs
...
- Future: Move zone management logic into bindy controller (eliminate bindcar dependency)
