...
> **Status:** ⛔ Not started — still a draft. No external-endpoint or gateway fields exist in `src/crd.rs`.
>
> *Migrated 2026-09-10 from the external roadmap set. Status verified against `fix-idempotency` @ `648ff7a`.*

---

**Original status (as written):** Draft
**Created:** 2026-03-30
...
spec:
  ...
  services:
    - name: bindcar-api
      remoteToken: "<tunnel-token>"  # placeholder; must match the rathole-client token.
                                     # Real value belongs in a Secret, never in a manifest.
...
token = "<tunnel-token>"        # placeholder — load from a Secret
local_addr = "127.0.0.1:8080"

[client.services.rndc]
token = "<rndc-tunnel-token>"   # placeholder — load from a Secret
