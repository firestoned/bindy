...
> **Status:** ⛔ Not started — `pub cluster_ref` is still in `src/crd.rs` at `:994`, `:3679` and `:3839`. Breaking CRD change; needs an ADR in [`docs/adr/`](../../docs/adr/) first.
>
> *Migrated 2026-09-10 from the external roadmap set. Status verified against `fix-idempotency` @ `648ff7a`.*

---

**Original status (as written):** 📋 Planning
**Date:** 2026-01-02
...
See [CHANGELOG.md](../../.claude/CHANGELOG.md) for full details.
```
