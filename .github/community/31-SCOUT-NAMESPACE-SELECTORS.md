...
> **Status:** 🔶 In progress — `namespace_selector` landed (#437) and is evaluated per event by `source_namespace_eligible()` (`src/scout.rs:2213` and 4 more call sites). The doc's actual goal — a `Namespace` **watch** so ineligible events never reach the controller — is not implemented; today every event still costs an API lookup.
>
> *Migrated 2026-09-10 from the external roadmap set. Status verified against `fix-idempotency` @ `648ff7a`.*

---

**Original status (as written):** Proposed
**Date:** 2026-03-28
