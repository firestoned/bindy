...
async fn stale_selector_matches_only_same_zone_records() {
    ...
    // An unrelated cluster publishing into a DIFFERENT zone. Before #474 was
    // fixed this was deleted on every reconcile.
    plant_arecord(&client, "live-north-beta", "north", ZONE_BETA).await;

    let selector = stale_arecord_label_selector("south", SOURCE_NAMESPACE, SOURCE_NAME, ZONE_ALPHA);
...
async fn every_resource_kind_selector_agrees_on_a_live_api_server() {
    ...
    for selector in [
        stale_arecord_label_selector("south", SOURCE_NAMESPACE, SOURCE_NAME, ZONE_ALPHA),
        stale_httproute_arecord_label_selector("south", SOURCE_NAMESPACE, SOURCE_NAME, ZONE_ALPHA),
        stale_tlsroute_arecord_label_selector("south", SOURCE_NAMESPACE, SOURCE_NAME, ZONE_ALPHA),
        stale_tcproute_arecord_label_selector("south", SOURCE_NAMESPACE, SOURCE_NAME, ZONE_ALPHA),
    ] {
        assert_eq!(
