...
scout-yaml: ## Generate single-file scout.yaml (RBAC + Deployment) for a given VERSION
	$(if $(VERSION),,$(error VERSION is required, e.g. make scout-yaml VERSION=v0.1.0))
	...
	@{ \
		...
		echo "#"; \
		echo "# DO NOT EDIT MANUALLY - Generated by 'make scout-yaml VERSION=$(VERSION)'"; \
		echo ""; \
		echo "# ============================================================"; \
		echo "# RBAC"; \
		echo "# ============================================================"; \
		cat deploy/scout/serviceaccount.yaml; \
		echo "---"; \
		cat deploy/scout/clusterrole.yaml; \
		echo "---"; \
		cat deploy/scout/clusterrolebinding.yaml; \
		echo "---"; \
		cat deploy/scout/role.yaml; \
		echo "---"; \
		cat deploy/scout/rolebinding.yaml; \
		echo ""; \
		echo "# ============================================================"; \
		echo "# Deployment"; \
		echo "# ============================================================"; \
...
kubesec: kubesec-install ## Scan Kubernetes manifests with Kubesec for security best practices
	...
	for file in deploy/operator/*.yaml deploy/operator/rbac/*.yaml examples/*.yaml; do \
		if [ -f "$$file" ]; then \
			echo "Scanning $$file..."; \
...
deploy-rbac: ## Deploy RBAC resources
	...
	kubectl apply -f deploy/operator/rbac/ -n $(NAMESPACE)
...
undeploy: ## Remove operator
	...
	kubectl delete -f deploy/operator/rbac/ -n $(NAMESPACE) || true
	kubectl delete -f deploy/operator/crds/ || true
...
kind-integration-test-ci: ## Run integration tests in CI mode (requires IMAGE_TAG env var)
	...
	@kubectl --context $(KIND_CONTEXT) replace --force -f deploy/operator/crds/ 2>/dev/null || kubectl --context $(KIND_CONTEXT) create -f deploy/operator/crds/
