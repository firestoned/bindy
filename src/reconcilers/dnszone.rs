...
pub async fn reconcile_dnszone(
    ...
    let (primary_count, secondary_count) = {
        ...
        let primary_ips =
            match primary::find_primary_ips_from_instances(&client, &instance_refs).await {
    ...
    let record_refs = match discovery::reconcile_zone_records(client.clone(), dnszone.clone()).await
    {
        Ok(refs) => {
            info!(
    ...
    status_updater.set_observed_generation(dnszone.metadata.generation);

    // Calculate expected counts to validate all instances were configured
    let expected_primary_count = primary::filter_primary_instances(&client, &instance_refs)
        .await
        .map(|refs| refs.len())
        .unwrap_or(0);
    let expected_secondary_count = secondary::filter_secondary_instances(&client, &instance_refs)
        .await
        .map(|refs| refs.len())
        .unwrap_or(0);
