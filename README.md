...
- Each BIND9 pod runs an **authenticated bindcar sidecar** (`ghcr.io/firestoned/bindcar:v0.8.0`). The operator calls its HTTP API with a `bindcar`-audience ServiceAccount token validated by Kubernetes TokenReview (Mode B). See the [bindcar migration guide](docs/src/operations/migration-guide.md).
